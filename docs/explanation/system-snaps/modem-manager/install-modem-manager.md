...
The ModemManager snap is currently available from the Ubuntu Store. It can be installed on any system that supports snaps but is only recommended on {ref}`Ubuntu Core <ref-index-ubuntu-core-documentation>` at the moment.
