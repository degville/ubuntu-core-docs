...
Some systems suppress _console-conf_ and its user creation, and on those systems, the `system-user` {ref}`assertion <ref-index-assertions>` permits the creation of local system users on specific devices by their associated {ref}`Brand store <ref-dedicated-snap-store-dedicated-snap-store>`.

To create a system user on these systems,  the `system-user` {ref}`assertion <ref-index-assertions>`, described on this page, needs to be embedded within a file called `auto-import.assert` that’s added to the system via the root directory of a removable USB storage device. For more details, see {ref}`System user <how-to-guides-manage-ubuntu-core-add-a-system-user>`.
